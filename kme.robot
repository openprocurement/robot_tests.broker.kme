...
${locator.edit.value.amount}    id = lots-start_price
${locator.edit.guarantee.amount}    id = dgf_sum
${locator.edit.minimalStep.amount}    id = lots-step
${locator.minNumberOfQualifiedBids}    id = auction-minNumberOfQualifiedBids
...
${locator.items[0].contractPeriod.startDate}    id = items[0].contractPeriod_startDate
${locator.items[0].contractPeriod.endDate}    id = items[0].contractPeriod_endDate
${locator.items[0].additionalClassifications[0].scheme}    id=items[0].additionalClassifications_scheme
${locator.items[0].additionalClassifications[0].id}    id=items[0].additionalClassifications_id
${locator.items[0].additionalClassifications[0].description}    id=items[0].additionalClassifications.description
${locator.items[1].additionalClassifications[0].description}    id=items[0].additionalClassifications.description
${locator.items[2].additionalClassifications[0].description}    id=items[0].additionalClassifications.description
...
${locator.items[1].contractPeriod.startDate}    id = items[1].contractPeriod_startDate
${locator.items[1].contractPeriod.endDate}    id = items[1].contractPeriod_endDate
...
${locator.items[2].contractPeriod.startDate}    id = items[2].contractPeriod_startDate
${locator.items[2].contractPeriod.endDate}    id = items[2].contractPeriod_endDate
...
Підготувати клієнт для користувача
    ...
    Run Keyword If    '${username}' != 'kme_Viewer'    Login    ${username}
...
Створити тендер
    ...
    ${minNumber}=    Get From Dictionary    ${ARGUMENTS[1].data}    minNumberOfQualifiedBids
    ...
    Input text    id=minNumber    ${minNumber}
...
Додати предмет
    ...
    Input text    id=items-contractperiod_startdate    ${start_date}
    Input text    id=items-contractperiod_enddate    ${end_date}
...
Завантажити документ
    ...
    ...    ${ARGUMENTS[2]} == ${TENDER}
    kme.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[2]}
...
Оновити сторінку з тендером
    ...
    Switch Browser    ${BROWSER_ALIAS}
    kme.Пошук тендера по ідентифікатору    ${ARGUMENTS[0]}    ${ARGUMENTS[1]}
...
Отримати інформацію із предмету
    ...
    ${return_value}=    Run Keyword And Return    kme.Отримати інформацію із тендера    ${username}    ${tender_uaid}    ${field_name}
    [Return]    ${return_value}
...
Отримати інформацію про procuringEntity.name
    ${return_value}=    Get Text    id = auction-procuringEntity_name
    [Return]    ${return_value}
...
Отримати інформацію про items[0].deliveryDate.endDate
    ...
    ${return_value}=    kme_service.convert_date    ${date_value}
    [Return]    ${return_value}
...
Відповісти на запитання
    ...
    kme.Перейти до сторінки запитань    ${username}  ${tender_uaid}
